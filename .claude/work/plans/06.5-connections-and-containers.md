...
Left as it is, deliberately: membership is per element, so a group inside a
frame is carried by its children rather than as a unit. Recorded in
`.ai/rules/canvas.md` rather than fixed, because deciding whether a group or
its children own membership is its own change.
